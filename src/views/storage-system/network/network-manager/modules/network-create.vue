...
<script setup lang="ts">
  ...
  const handleSelectionChange = (selectedRows: NetworkDeviceInterface[]) => {
    ...
    if (selectedRows.length > 1) {
      ...
      spareTableRef.value.toggleRowSelection(latestRow, true)
    }
    selectNetworkDevice.value = selectedRows[0]
  }
  ...
  const nextStep = () => {
    ...
    if (currentStep.value === 1) {
      formData.value.name = selectNetworkDevice.value.device
      Object.assign(formData.value, selectNetworkDevice.value)
      handleConfirm()
    } else {
      if (!selectNetworkDevice.value) {
