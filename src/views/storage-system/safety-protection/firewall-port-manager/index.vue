      ...
      <div class="right-content art-full-height" style="margin-bottom: 40px">
        <ElCard class="art-table-card" shadow="never">
          <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="refreshData">
            <template #left>
              <ElSpace wrap>
                <ElButton @click="showGroupDialog('add')">添加端口</ElButton>
...
<script setup lang="ts">
  ...
  import { fetchAddGroup, fetchDelGroups, fetchEditGroup } from '@/api/system-manage'
  import { fetchParesPortList } from '@/api/safety-protection'
  import UserDialog from './modules/user-dialog.vue'
  import SysGroup = Api.Sys.SysGroup
  import GroupDialog from '@views/storage-system/safety-protection/firewall-port-manager/modules/group-dialog.vue'
    ...
    core: {
      ...
      columnsFactory: () => [
        ...
        },
        {
          ...
          formatter: (row: any) => {
            if (row.enable) {
              return h(ElTag, { type: 'success' }, () => '开启')
            } else {
              return h(ElTag, { type: 'danger' }, () => '关闭')
            }
          }
        },
        {
